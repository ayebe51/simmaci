...
try {
    ...
    if ($fathurLayansari && $skCilopadang) {
        $fathurCilopadang = Teacher::withoutGlobalScopes()->where('nomor_induk_maarif', '113403337')->first();
        if (!$fathurCilopadang) {
            // Jika profilnya belum ada (mungkin terhapus), buatkan profil darurat
            $fathurCilopadang = Teacher::create([
                'nama' => 'FATHURROHMAN, S.Pd.I',
                'nomor_induk_maarif' => '113403337',
                'school_id' => 83, // Cilopadang
                'status' => 'Draft',
                'is_verified' => false
            ]);
            echo "   ⚠️ Profil Cilopadang (NIM: 113403337) dibuat ulang.\n";
        }
        
        // Pindahkan SK ke guru aslinya
        $skCilopadang->teacher_id = $fathurCilopadang->id;
        $skCilopadang->save();
        echo "   🔄 SK ID 433 berhasil dikembalikan ke Profil Cilopadang (NIM 113403337).\n";
        $movedSks++;
        
        // Kembalikan profil Layansari ke sekolahnya
        $fathurLayansari->school_id = 64;
        $fathurLayansari->save();
        echo "   🔄 Profil Layansari (NIM 113401741) berhasil dikembalikan ke MI Ma'arif 02 Layansari.\n";
        $fixedTeachers++;
        echo "--------------------------------------------------------\n";
    }
    ...
    echo "Total SK yang berhasil dipindahkan ke Profil Asli: {$movedSks}\n";
