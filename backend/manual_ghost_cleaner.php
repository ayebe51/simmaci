...
try {
    foreach ($instructions as $nim => $targetSchoolId) {
        ...
        if ($targetSchoolId === 'SPLIT') {
            echo " ➡️ Memecah profil guru untuk mengajar di banyak sekolah...\n";
            foreach ($sks as $i => $sk) {
                if ($i === 0) {
                    // Profil asli dipertahankan untuk SK pertama
                    $teacher->school_id = $sk->school_id;
                    $teacher->save();
                    echo "    ✅ Profil Asli di-set untuk School ID: {$sk->school_id}\n";
                } else {
                    // Buat profil kloningan untuk SK-SK berikutnya
                    $newTeacher = $teacher->replicate();
                    $newTeacher->school_id = $sk->school_id;
                    $newTeacher->save();
                    
                    // Pindahkan SK ke guru yang baru
                    $sk->teacher_id = $newTeacher->id;
                    $sk->save();
                    echo "    ✅ Profil Kloningan dibuat untuk School ID: {$sk->school_id} (New Teacher ID: {$newTeacher->id})\n";
                    $clonedTeachers++;
                }
            }
            echo "--------------------------------------------------------\n";
        ...
        if ($teacher->school_id != $targetSchoolId) {
            ...
            // Kembalikan ke nama aslinya sesuai SK dari target sekolah (jika ada)
    ...
    echo "Total Profil Guru yang diduplikasi (Split): {$clonedTeachers}\n";
