...
export default function StudentPhotoUpload({ photoId, onPhotoUploaded, onRemovePhoto }: StudentPhotoUploadProps) {
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      setIsUploading(true);
      const result = await mediaApi.upload(file, 'students');
  ...
  return (
    <div className="flex flex-col gap-3 items-center p-4 border rounded-lg bg-slate-50 border-dashed border-slate-300">
      <div className="relative w-32 h-40 bg-slate-200 rounded overflow-hidden flex items-center justify-center shadow-sm border">
        {photoId ? (
          <img src={photoId} alt="Foto Siswa" className="w-full h-full object-cover" />
        ) : (
      ...
      <div className="flex gap-2">
        ...
        {photoId && onRemovePhoto && (
          <Button variant="destructive" size="icon" type="button" className="w-9 h-9" onClick={onRemovePhoto}>
            <X className="h-4 w-4" />
          </Button>
        )}
      </div>
      ...
      <input
        ...
        aria-label="Upload Foto Siswa"
        title="Upload Foto Siswa"
      />
      <p className="text-[10px] text-muted-foreground text-center">
        Format: JPG/PNG, Max 2MB.<br />Rasio 3:4 (Pas Foto).
      </p>
    </div>
  );
}
